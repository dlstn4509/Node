...
router.get('/:id', async (req, res, next) => {
	...
	let sql, values;
	try {
		sql = `SELECT B.*, F.realName, F.saveName, F.mimetype, F.fid FROM board B LEFT JOIN files F ON B.id = F.fid WHERE B.status > '0' AND F.status > '0' AND B.id=?`
